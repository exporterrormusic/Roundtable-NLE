/*
 * AudioSync.cpp — 5-step audio sync workflow panel implementation.
 */
...
// ─── Event filter: route spacebar / J / K / L to the selected clip ──────────
...
bool AudioSync::eventFilter(QObject* watched, QEvent* event)
{
                ...
                if (card) {
                    ...
                    for (size_t i = 0; i < widgets.size(); ++i) {
                        if (widgets[i] == card) {
                            ...
                            // Apply card highlight (no scroll — only highlight)
    ...
    if (event->type() == QEvent::KeyPress) {
        ...
        // ── Space key ───────────────────────────────────────────────
        // Always handle Space here.  Never pass through to the widget’s
        // keyPressEvent—focus can end up on the QListWidget itself rather
        ...
        {
            if (m_forwardingKey)
                return false;  // inside sendEvent — let it reach the target widget
...
// ── Safety-net: catch Space that propagates up from unfiltered children ──
